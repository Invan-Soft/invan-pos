...
- `VAT` — `(Price − Discount − Other) × VATPercent / (100 + VATPercent)`. База НДС — сумма **после скидки** (официальный пример: Price 100000, Discount 50000, VATPercent 12 → VAT 5357). Если результат отрицательный — `0`. До 2026-09-24 `Discount` не вычитался — это был баг: по товарам со скидкой НДС уходил с цены без скидки.
