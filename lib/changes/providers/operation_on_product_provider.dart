...
//   // Chegirma summasi (sum ko'rinishida): basePrice - price
//   String get discountSumStr => MoneyFormatter.inputMoneyFormatter
//       .format((basePrice - target.price).clamp(0.0, double.infinity));
...
//   bool get _isMarking =>
//       target.marking ||
//       target.mxik.startsWith('022') ||
//       target.mxik.startsWith('024');
...
//   //   if (priceManuallyEdited) {
//   //     // ✅ Kuchaytirilgan tozalash
//   //     target.discount.clear();
//   //     target.productDiscount.clear();           // ← yangi qator
//   //     target.discountPercent = 0;
//   //     target.singleDiscount = 0;
//   //     target.isPriceOnlyChanged = true;
//   //     target.isPriceChanged = true;

//   //     print("🔹 OperationOnProductProvider → Narx qo'lda o'zgartirildi. Discount TOZALANDI");
//   //     print("   discount list length: ${target.discount.length}");
//   //     print("   productDiscount length: ${target.productDiscount.length}");
//   //   }
...
//     // ← BU IKKI QATORNI QO‘SHING (eng muhim!)
...
//     print("🔹 Manual price edit → ALL discounts fully cleared (discountPercent = 0)");
//   }

//   // Faqat onlyPriceIsEdited bo‘lsa narxni yangilaymiz
//   if (onlyPriceIsEdited == true) {
//     target.price = target.onlyPrice;
//     target.isPriceOnlyChanged = true;
//   }
...
//  void onPriceChanged(num v) {
//   if (!(_currentEmployee?.access?.editPrice ?? false)) return;
//   if (!_chekFor122(target.price)) return;
...
//   // Asl narxni saqlab qolamiz
//   double originalPrice = target.realPrice.toDouble();

//   // Chegirma % ni qayta hisoblaymiz
//   if (originalPrice > 0) {
//     target.discountPercent = ((originalPrice - newPrice) / originalPrice) * 100;
//   } else {
//     target.discountPercent = 0;
//   }

//   // Discount listlarni tozalaymiz
//   target.discount.clear();
//   target.productDiscount.clear();
...
// //   // Top inputda narx o'zgartirilsa - bu asl narxni o'zgartirish, chegirma EMAS
// //   target.onlyPrice = newOnlyPrice;
// //   target.price = newOnlyPrice;
// //   target.realPrice = newOnlyPrice;
// //   target.discountPercent = 0;
...
//  void onlyPriceChanged(num v) {
//   if (!(_currentEmployee?.access?.editPrice ?? false)) return;
//   if (!_chekFor122(target.onlyPrice)) return;
...
//   // Yangi narx (chegirmadan keyingi)
//   target.onlyPrice = newOnlyPrice;
//   target.price = newOnlyPrice;

//   // Asl narxni (realPrice) hech qachon o'zgartirmaymiz!
//   // basePrice = target.realPrice bo'lib qoladi

//   // Chegirma summasini hisoblaymiz va Price dan oshirmaymiz
//   double originalPrice = target.realPrice.toDouble();
//   double discountSum = (originalPrice - newOnlyPrice).clamp(0.0, originalPrice);

//   // Chegirma % ni to'g'ri hisoblaymiz
//   if (originalPrice > 0) {
//     target.discountPercent = (discountSum / originalPrice) * 100;
//   } else {
//     target.discountPercent = 0;
//   }

//   // Discount listlarni tozalaymiz (soliq uchun majburiy)
//   target.discount.clear();
//   target.productDiscount.clear();
...
//             if (isSelectedAlls[_selectedInput]) _switchSelectedAlls();
//             target.discountPercent = 0;
//             target.price = basePrice;
//             target.onlyPrice = basePrice;
//             notifyListeners();
//             return;
//           }
...
//             return;
//           }
//           summa = _cutBack(target.onlyPrice);
//           final newPrice = summa.toDouble();
//           target.onlyPrice = newPrice;
//           target.price = newPrice;
...
//   if (v == 10) {
//     newDiscountPercent = (target.discountPercent ?? 0) / 10;
//   } else if (isSelectedAlls[_selectedInput] || (target.discountPercent ?? 0) == 0) {
//     if (isSelectedAlls[_selectedInput]) _switchSelectedAlls();
//     newDiscountPercent = v.toDouble();
//   } else {
//     newDiscountPercent = _extend(target.discountPercent ?? 0, v).toDouble();
//   }

//   // Chegirma 100% dan oshmasin
//   if (newDiscountPercent > 100) newDiscountPercent = 100.0;

//   target.discountPercent = newDiscountPercent;

//   // Asl narxni olamiz
//   double originalPrice = target.realPrice.toDouble();

//   // Chegirma summasi
//   double discountSum = (originalPrice * newDiscountPercent) / 100;

//   // Yangi narx (hech qachon manfiy bo'lmasin)
//   double newPrice = (originalPrice - discountSum).clamp(0.0, originalPrice);
...
//     if (!_isMarking && target.soldBy == "box") {
